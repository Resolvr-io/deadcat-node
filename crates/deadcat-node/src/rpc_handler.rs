...
use deadcat_rpc::{
    ...
    MarketSnapshot, NodeInfo, PackageRegistrationReceipt, PageRequest, RecoveryHintPage,
    RecoveryHintRecord, RegistrationReceipt, Request, Response, RpcError, RpcErrorCode,
    SnapshotCursor, SnapshotMetadata, SubscriptionEnd, TransactionEvidence,
};
use deadcat_types::{
                ...
                match store.events_after(cursor, 256) {
                    Ok(events) => {
                        ...
                        for stored in events {
                            ...
                            if event_matches(&stored, &filter)
                                && sender
...
fn contract_view(record: ContractRecord) -> ContractView {
    let ContractParameters::BinaryMarket(params) = record.params;
    let parameters = deadcat_rpc::ContractParametersView::BinaryMarket { params };
    let ContractState::BinaryMarket(state) = record.state;
    let state = deadcat_rpc::ContractStateView::BinaryMarket { state };
    ContractView {
...
fn event_matches(event: &StoredEventEnvelope, filter: &EventFilter) -> bool {
    match filter {
