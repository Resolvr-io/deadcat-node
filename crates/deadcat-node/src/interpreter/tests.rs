...
fn composed_market_issuance(
    ...
    for issuance in issuances {
        let ContractState::BinaryMarket(before) = issuance.record.state else {
            panic!("market fixture state");
        };
...
#[test]
fn destructive_replay_revalidates_retained_market_without_maker_orders() {
    let directory = tempfile::tempdir().expect("tempdir");
    let store = Store::open(directory.path().join("deadcat.redb")).expect("store");
    let genesis = anchor(0, 0x01);
